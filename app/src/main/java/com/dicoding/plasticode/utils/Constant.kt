package com.dicoding.plasticode.utils

object Constant {
    const val DELAY_SPLASH_SCREEN = 2000L
    const val MAPS_URL = "https://maps.googleapis.com/"
    const val MAPS_IMAGE_URL = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference="
    const val MAPS_RADIUS = 50000
    const val MAPS_API_KEY = "AIzaSyBcyCQmx3ONVzA48olnKcKuUeA6eu_MZHI"
    const val MAPS_KEYWORD = "daur ulang plastik"
    const val AUTH_PREFERENCES = "auth"
    const val STATE_PREFERENCES = "state"
    const val TYPE_HEADERS = "Authorization"
    const val BASE_URL = "https://plasticode-vxoopzmopq-et.a.run.app/api/"
    const val ID_USER = "idUser"
    const val NAME_USER = "nameUser"
    const val EMAIL_USER = "emailUser"
    const val THEME_KEY = "theme"
}